...
int Offset(const uint16_t dims_data1, const uint16_t dims_data2, const uint16_t dims_data3, int i0, int i1, int i2,
           int i3) {
...
void Conv2D::forward(const Matrix3D<float>& input, Matrix3D<float>& output) {
    ...
    const int filter_input_depth = params.weight.m_dim_w, filter_width = params.weight.m_dim_x,
              filter_height = params.weight.m_dim_y;
    ...
    const int dilation_width_factor = params.dilation_width_factor,
              dilation_height_factor = params.dilation_height_factor;
    ...
    // assert(input.m_dim_z == params.weight.m_dim_z);
        ...
        for (int out_y = 0; out_y < output_height; ++out_y) {
            ...
            for (int out_x = 0; out_x < output_width; ++out_x) {
                ...
                for (int out_channel = 0; out_channel < output_depth; ++out_channel) {
                    ...
                    for (int filter_y = 0; filter_y < filter_height; ++filter_y) {
                        ...
                        for (int filter_x = 0; filter_x < filter_width; ++filter_x) {
                            ...
                            for (int in_channel = 0; in_channel < filter_input_depth; ++in_channel) {
                                ...
                                float filter_value = filter_data[Offset(filter_height, filter_width, input_depth,
                    ...
                    if (has_bias) {
                        bias_value = bias.m_data[out_channel];
                    }
