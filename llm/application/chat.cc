...
bool isLLaMA(std::string s) {
    ...
    if (s.substr(0, LLaMA_prefix.size()) == LLaMA_prefix || s.substr(0, CodeLLaMA_prefix.size()) == CodeLLaMA_prefix ||
        s == "7b" || s == "13b")
        return true;
    else
        return false;
...
int main(int argc, char* argv[]) {
    ...
    if (argc >= 3 && argc <= 5) {
        ...
        if (isLLaMA(target_model) || isCodeLLaMA(target_model) || isStarCoder(target_model) || isLLaVA(target_model) ||
            isVILA(target_model) || isMistral(target_model)) {
    ...
    if (isLLaMA3(target_model)) {
        ...
        if (use_voicechat) {
            ...
            std::cout << "- Please use this mode in a quiet environment to have a better user experience and avoid "
                         "speech misdetection."
                      << std::endl;
        ...
        if (format_id == FP32) {
            ...
            while (true) {
                ...
                if (first_prompt) {
                    input =
                        "A chat between a curious human (\"Human\") and an artificial intelligence assistant "
                        "(\"Assistant\"). The assistant gives detailed, helpful, and polite answers to the human's "
                        "questions.\n\nHuman: " +
                        input + "\nAssistant: ";
                    first_prompt = false;
                } else {
                ...
                LLaMA3Generate(m_path, &model, LLaMA_FP32, input, generation_config, "models/llama3_vocab.bin", true,
                               false);
            }
            ...
            while (true) {
                ...
                LLaMA3Generate(m_path, &model, LLaMA_INT4, input, generation_config, "models/llama3_vocab.bin", true,
                               use_voicechat);
            }
        } else {
            std::cout << std::endl;
        ...
        if (format_id == FP32) {
            ...
            while (true) {
                ...
                LLaMAGenerate(m_path, &model, LLaMA_FP32, input, generation_config, "models/llama_vocab.bin", true,
                              false);
            }
        } else if (format_id == INT4) {
            m_path = "INT4/" + m_path;
            ...
            while (true) {
                ...
                LLaMAGenerate(m_path, &model, LLaMA_INT4, input, generation_config, "models/llama_vocab.bin", true,
                              use_voicechat);
            }
        } else {
            std::cout << std::endl;
        ...
        if (format_id == FP32) {
            ...
            while (true) {
                ...
                GPTBigCodeGenerate(m_path, &model, StarCoder_FP32, input, generation_config,
                                   "models/starcoder_vocab.bin", true);
            }
        } else if (format_id == INT4) {
            m_path = "INT4/" + m_path;
            ...
            while (true) {
                ...
                GPTBigCodeGenerate(m_path, &model, StarCoder_INT4, input, generation_config,
                                   "models/starcoder_vocab.bin", true);
            }
        } else {
            std::cout << std::endl;
        ...
        if (format_id == FP32) {
            Fp32CLIPVisionTransformer clip_model =
                Fp32CLIPVisionTransformer(clip_m_path, get_opt_model_config(clip_model_id), true);
            Fp32LlamaForCausalLM llama_model = Fp32LlamaForCausalLM(llama_m_path, get_opt_model_config(llama_model_id));
            ...
            while (true) {
                ...
                LLaVAGenerate(llama_m_path, &llama_model, clip_m_path, &clip_model, VILA_FP32, input, img_path,
                              generation_config, "models/llama_vocab.bin", true, false, true);
            }
        } else if (format_id == INT4) {
            Fp32CLIPVisionTransformer clip_model =
                Fp32CLIPVisionTransformer(clip_m_path, get_opt_model_config(clip_model_id), true);
            ...
            while (true) {
                ...
                LLaVAGenerate(llama_m_path, &llama_model, clip_m_path, &clip_model, VILA_INT4, input, img_path,
                              generation_config, "models/llama_vocab.bin", true, use_voicechat, true);
            }
        } else {
            std::cout << std::endl;
        ...
        if (format_id == FP32) {
            ...
            while (true) {
                ...
                MistralGenerate(m_path, &model, LLaMA_FP32, input, generation_config, "models/mistral_vocab.bin", true,
                                false);
            }
        } else if (format_id == INT4) {
            m_path = "INT4/" + m_path;
            ...
            while (true) {
                ...
                MistralGenerate(m_path, &model, LLaMA_INT4, input, generation_config, "models/mistral_vocab.bin", true,
                                use_voicechat);
            }
        } else {
            std::cout << std::endl;
